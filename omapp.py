...
# 2. Construct the full path to model.pkl and scaler.pkl
...
st.write("Full scaler path:", scaler_path)
st.write("Model file exists:", os.path.exists(model_path))
st.write("Scaler file exists:", os.path.exists(scaler_path))

# 4. Load the model using the full path
with open(model_path, "rb") as file:
    model = pickle.load(file)

# 5. Load the scaler using the full path
...
dataset = pd.read_csv(r"C:\Users\om718\Downloads\ipl_batsmen_current_players_no_team.csv")
